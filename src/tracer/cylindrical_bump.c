...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/01/11 02:29:28 by smatsuo           #+#    #+#             */
/*   Updated: 2024/01/11 02:30:17 by smatsuo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vec3	get_cylindrical_normal(t_object *cyl, t_hit_record *rec, t_vec3 uv)
{
	...
	const t_vec3			normal = vec3_normalize(vec3_sub(diff,
				vec3_project(diff, conf.axis)));
	...
	p.first = vec3_normalize(vec3_sub(
				uv_to_cylindrical(cyl, vec3(uv.x + map.pixel_w, uv.y, uv.z)),
				uv_to_cylindrical(cyl, vec3(uv.x - map.pixel_w, uv.y, uv.z))));
	p.second = vec3_normalize(vec3_sub(
				uv_to_cylindrical(cyl, vec3(uv.x, uv.y + map.pixel_h, uv.z)),
				uv_to_cylindrical(cyl, vec3(uv.x, uv.y - map.pixel_h, uv.z))));
