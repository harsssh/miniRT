...
{
	const t_sphere_conf	conf = *(t_sphere_conf *)sphere->conf;
	const t_vec3		normal = vec3_div(vec3_sub(rec->point, conf.center), conf.radius);
	const t_height_map	map = sphere->material.height_map;
	t_vec_pair			p;

	if (map.img == NULL)
		return (normal);
	p.first = vec3_normalize(vec3_sub(uv_to_spherical(sphere, vec3(uv.x + map.pixel_w, uv.y, uv.z)), uv_to_spherical(sphere, vec3(uv.x - map.pixel_w, uv.y, uv.z))));
	p.second = vec3_normalize(vec3_sub(uv_to_spherical(sphere, vec3(uv.x, uv.y + map.pixel_h, uv.z)), uv_to_spherical(sphere, vec3(uv.x, uv.y - map.pixel_h, uv.z))));
